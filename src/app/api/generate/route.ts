...
export async function POST(req: Request) {
  try {
    const { referenceBase64, avatarBase64, bagBase64, quality } = await req.json();
    ...
    console.log("Paso 1: Analizando imagen de referencia con Gemini 2.5 Flash...");
...
Crear descripciones visuales reutilizables para marketing, branding y análisis estético, combinando producto, composición, postura corporal, jerarquía visual y precisión editorial con alto nivel profesional.`;
    ...
    let imageAnalysisJson = analysisResponse.text || '{}';
    ...
    const response = await ai.models.generateContent({
      ...
      contents: [
        ...
        The FIRST image is the reference background and body pose. \n
        The SECOND image is the character's face (avatar). \n
        The THIRD image is the product bag.\n
        Follow these steps:\n
        1. Base your entire new image on the exact background, lighting, and pose of the FIRST image.\n
        ...
        Use the following structured visual description of the reference image to accurately guide your generation, maintaining the exact layout, lighting, and composition described here:\n
        ${imageAnalysisJson}
        \n
        Return ONLY the resulting generated image inline, no text.`,
        {
          inlineData: {
            data: referenceClean,
            mimeType: 'image/jpeg' // we assume jpeg as default clean up fallback for generative requests
          }
        },
        {
          inlineData: {
    ...
    let parsedJson = {};
    try {
      parsedJson = JSON.parse(imageAnalysisJson);
    } catch (e) {
      console.warn("Could not parse imageAnalysisJson, returning raw string or empty object", imageAnalysisJson);
    }
